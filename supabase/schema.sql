...
alter table public.mood_logs add column if not exists stress_level smallint;
alter table public.mood_logs drop constraint if exists mood_logs_stress_level_chk;
alter table public.mood_logs add constraint mood_logs_stress_level_chk
...
-- Server-only monthly allowance claims for AI workout generation.
create table if not exists ai_workout_generation_claims (
  id uuid primary key default gen_random_uuid(),
...
on ai_workout_generation_claims(user_id, usage_month, status);
...
-- Canonical pillar score calculation for a non-overlapping time period. The
-- caller supplies both date bounds (daily logs) and timestamp bounds (events).
...
-- querying authenticated user applies. Exposed to authenticated clients and service-role reads.
